...
class Osztas_teszt2 {
	...
	function Generate($level) {
		...
		$question = 'Ha egy számban '.$rules[$num1].', akkor a szám osztható $'.$num2.'$-'.With($num2).'.';
	...
	function Hints($num1, $num2, $rules) {
		...
		for ($i=1; $i < 5; $i++) { 
			if ($i*$num1 % $num2 == 0) {
				$page[] = '<ul><li>$'.strval($i*$num1).'$: ez a szám osztható $'.$num2.'$-'.With($num2).'.</li></ul>';
			} else {
				$page[] = '<ul><li>$'.strval($i*$num1).'$: ez a szám nem osztható $'.$num2.'$-'.With($num2).'.</li></ul>';
				break;
			}
		}

		if ($num1 == $num2 || $num1 % $num2 == 0) {
			$page[] = 'Könnyen meg lehet mutatni, hogy az összes ilyen szám osztható $'.$num2.'$-'.With($num2).'.';

...
class Sine_range {
	...
	function DrawSineFunction($a1=0, $b1=1, $a2=0, $b2=0, $value_min=-1, $value_max=1, $drawRange=0) {
		...
		if ($b2) {
			for ($i=0; $i < $width; $i++) {
				...
				$svg .= $this->DrawLine($i, $yval1, $i+1, $yval2, 'grey', 1);
			}
		}
	...
	function DrawRange($a1, $b1, $yunit_length, $yunit_original, $originy, $width) {
		...
		$svg = $this->DrawLine($xval, $yval1, $xval, $yval2, 'blue', 2);
		$svg .= $this->DrawLine($xval, $yval1, $xval-5, $yval1+5, 'blue', 2);
		$svg .= $this->DrawLine($xval, $yval1, $xval+5, $yval1+5, 'blue', 2);
		$svg .= $this->DrawLine($xval, $yval2, $xval-5, $yval2-5, 'blue', 2);
		$svg .= $this->DrawLine($xval, $yval2, $xval+5, $yval2-5, 'blue', 2);
		return $svg;
	}
}
